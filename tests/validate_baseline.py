...
COMPLEXITY_JSON = FIXTURE_DIR / "complexity.json"


def _load_complexity_data() -> dict:
    """Load complexity.json if available; return empty dict otherwise."""
...
_COMPLEXITY_DATA: dict = _load_complexity_data()

# Map complexity_tier -> difficulty label used historically in FixtureInfo
_TIER_TO_DIFFICULTY = {
    "simple": "easy",
    "moderate": "medium",
    "complex": "hard",
...
class FixtureInfo:
    ...
    def __post_init__(self):
        # Override difficulty from computed complexity.json if available
        if self.name in _COMPLEXITY_DATA:
            tier = _COMPLEXITY_DATA[self.name].get("complexity_tier", "simple")
            self.difficulty = _TIER_TO_DIFFICULTY.get(tier, "easy")
...
def calculate_summary(results: list[FixtureResult]) -> BaselineSummary:
    ...
    if scored:
        ...
        avg_ornament = sum(r.ornament_accuracy for r in scored) / len(scored)
        avg_grace = sum(r.grace_note_accuracy for r in scored) / len(scored)
    else:
        avg_note = avg_pitch = avg_rhythm = avg_overall = 0.0
        avg_measure = avg_key = avg_time = avg_ornament = avg_grace = 0.0
    ...
    return BaselineSummary(
        ...
        avg_ornament_accuracy=avg_ornament,
...
def generate_markdown_report(results: list[FixtureResult], summary: BaselineSummary, model: str = "claude-sonnet-4-6") -> str:
    ...
    lines = [
        ...
        f"| Ornament Accuracy | **{summary.avg_ornament_accuracy:.1f}%** | - |",
        f"| Grace Note Accuracy | **{summary.avg_grace_note_accuracy:.1f}%** | - |",
        ...
        "## Per-Fixture Results",
        "",
        "| Fixture | Status | Note Acc | Pitch Acc | Rhythm Acc | Ornament Acc | Grace Acc | Overall Acc | Notes | Time |",
        "|---------|--------|----------|-----------|-------------|--------------|-----------|-------------|-------|------|",
    ]
    ...
    for r in sorted_results:
        ...
        lines.append(f"| {r.fixture.name} | {status} | {r.note_accuracy:.1f}% | {r.pitch_accuracy:.1f}% | {r.rhythm_accuracy:.1f}% | {r.ornament_accuracy:.1f}% | {r.grace_note_accuracy:.1f}% | {r.overall_accuracy:.1f}% | {notes} | {time_str} |")
...
def print_console_report(results: list[FixtureResult], summary: BaselineSummary) -> None:
    ...
    table.add_column("Ornament %", justify="right")
    ...
    for r in sorted(results, key=lambda x: x.fixture.name):
        ...
        notes = f"{r.matched_note_count}/{r.gt_note_count}" if r.gt_note_count else "-"
        time_str = f"{r.duration_seconds:.1f}s"
        table.add_row(r.fixture.name, status, notes, f"{r.pitch_accuracy:.0f}%", f"{r.rhythm_accuracy:.0f}%", f"{r.ornament_accuracy:.0f}%", f"{r.grace_note_accuracy:.0f}%", f"{r.overall_accuracy:.0f}%", time_str)
...
def main(fixture, model, output, quiet, list_fixtures):
    ...
    json_data = {
        ...
        "summary": {
            ...
            "avg_ornament_accuracy": summary.avg_ornament_accuracy,
            "avg_grace_note_accuracy": summary.avg_grace_note_accuracy,
        ...
        "results": [
            {
                ...
                "error": r.error,
                "ornament_accuracy": r.ornament_accuracy,
                "grace_note_accuracy": r.grace_note_accuracy,
            }
            for r in results
        ],
    }
